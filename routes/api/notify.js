...
router.post('/', function (req, res) {
    ...
    if(req.body.to) {
        if(/^\w+@[a-zA-Z_]+?\.[a-zA-Z]{2,3}$/.test(req.body.to)) {
            ...
            push({email: req.body.to}, notification);
            mail({email: req.body.to}, emailOptions);
            res.json({
                status: 200,
                msg: "Success",
            });
            ...
            eventsService.findOne({route: req.body.to})
            .then(function(event){
                push({events: event._id}, notification);
                mail({events: event._id}, emailOptions);
                res.json({
                    status: 200,
                    msg: "Success",
                });
